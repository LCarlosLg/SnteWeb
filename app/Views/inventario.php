                    ...
                    <div class="card-header d-flex justify-content-between align-items-center">
                        ...
                        <div>
                            <form class="d-inline-block me-2" method="get" action="<?= base_url('inventario') ?>">
                                <input class="form-control form-control-sm d-inline-block w-auto" name="buscar"
                                    placeholder="Buscar..." value="<?= esc($buscar) ?>">
                            </form>
                            <button class="btn btn-success btn-sm" data-bs-toggle="modal"
                                data-bs-target="#modalProducto" onclick="nuevoProducto()">+ Agregar</button>
                                        ...
                                        <tr>
                                            <td>
                                                <?php if ($producto['imagen']): ?>
                                                    <img src="<?= base_url('inventario/mostrarImagen/' . $producto['imagen']) ?>"
                                            ...
                                            <td>
                                                <button class="btn btn-warning btn-sm"
                                                    onclick="editarProducto(<?= htmlspecialchars(json_encode($producto), ENT_QUOTES, 'UTF-8') ?>)">Editar</button>
                                                <a href="<?= base_url('inventario/eliminar/' . $producto['id']) ?>"
                                                    class="btn btn-danger btn-sm" onclick="return confirm('¿Borrar?')">X</a>
                                            </td>
                                        </tr>
                                    <?php endforeach; ?>
                                <?php else: ?>
                            ...
                            <tbody>
                                <?php if (!empty($pedidos)): ?>
                                    <?php foreach ($pedidos as $pedido): ?>
                                        ...
                                        $textoDireccion = is_array($dir) ? ($dir['calle'] ?? 'N/A') . ', ' . ($dir['colonia'] ?? '') : 'N/A';
                                        ?>
                                        <tr>
                                            ...
                                            <td>
                                                <ul class="list-unstyled mb-0 small">
                                                    <?php if (!empty($pedido['productos_comprados'])): ?>
                                                        <?php foreach ($pedido['productos_comprados'] as $item): ?>
                                                            <li>
                                                        ...
                                                        <?php endforeach; ?>
                                                    <?php else: ?>
                                                        <li class="text-muted fst-italic">Sin detalles</li>
                                                    <?php endif; ?>
                                                </ul>
                                            </td>

                                            <td><?= date('d/m/y H:i', strtotime($pedido['fecha_pedido'])) ?></td>
                                            <td><span class="badge bg-success"><?= esc($pedido['estado']) ?></span></td>
                                            <td><small><?= esc($textoDireccion) ?></small></td>
                                        </tr>
                                    <?php endforeach; ?>
                                <?php else: ?>
                                    <tr>
                        ...
                        <form action="<?= base_url('inventario/agregar') ?>" method="post" enctype="multipart/form-data"
                            ...
                            <div class="modal-body">
                                ...
                                <div class="mb-3">
                                    ...
                                    <small class="form-text text-muted">Si quieres cambiar la imagen, selecciona un
                                        archivo
                                        nuevo.</small>
                                </div>
                            </div>
